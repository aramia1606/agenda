...
if __name__ == "__main__":
    ...
    doted_canvas(c, 1, .02)
    # for i in range(int(HAUTEUR*cm)): 
        # c.drawRightString(1*cm, i*cm,f"{i}")
    months = [i+1 for i in range(12)]
    year = 2026
    for i,month in enumerate(months[:6]):
        draw_month_calendar(c, year, month,2*cm, 1*cm+ (7*cm*i)) 
        #c.line( 2*cm,(8*cm*i), LARGEUR-3*cm,(9*cm*i))
    ...
    # for i,month in enumerate(months[:6]):
    #     draw_month_calendar(c, year, month,2*cm, 1*cm+ (7*cm*i)) 
    #     c.line( 2*cm, 1*cm+ (7*cm*i) , LARGEUR-3*cm, 10*cm+ (8*cm*i))
